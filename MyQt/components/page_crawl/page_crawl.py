...
from PyQt5.QtWidgets import QWidget, QApplication, QScrollArea, QFileDialog
...
from weather_predict.seaborn_analysis_test import create_weather_charts_, create_blank_charts_
...
class CrawlWidgets(SiPage):
    def __init__(self, *args, **kwargs):
        ...
        self.chart_container2 = None

    def _setup_ui(self):
        self.setPadding(64)
        self.setScrollMaximumWidth(1000)
        self.setScrollAlignment(Qt.AlignLeft)
        self.setTitle("数据爬取与分析模块")
        ...
        with self.titled_widgets_group as group:
            ...
            self.view_button.clicked.connect(self.select_file_plt)

            # 创建图表容器
            self.chart_container = OptionCardPlaneForWidgetDemos(self)
            self.chart_container.setTitle("多维度气象趋势分析")
        ...
        with self.titled_widgets_group as group:
            ...
            self.view_button2 = SiPushButtonRefactor(self)
            self.view_button2.setText("选择要查看的数据")
            self.view_button2.clicked.connect(self.select_file_seaborn)

            # 创建图表容器
            self.chart_container2 = OptionCardPlaneForWidgetDemos(self)
            self.chart_container2.setTitle("时间序列与相关性分析")

            # 加载Matplotlib图表
            self.charts2 = create_blank_charts_(self)
            self.charts2.setMinimumSize(1400, 1800)  # 根据实际图表尺寸调整

            # 添加滚动支持
            self.scroll2 = QScrollArea()
            self.scroll2.setWidgetResizable(True)
            self.scroll2.setMinimumWidth(1000)  # 设置最小宽度
            self.scroll2.setWidget(self.charts2)

            self.chart_container2.body().addWidget(self.view_button2)
            self.chart_container2.body().addWidget(self.scroll2)
            self.chart_container2.adjustSize()

            group.addWidget(self.chart_container2)
    ...
    def seaborn_csv_data(self, file_path):
        try:
            # 加载Matplotlib图表
            self.charts2 = create_weather_charts_(self, file_path)
            self.charts2.setMinimumSize(1000, 2500)  # 根据实际图表尺寸调整
            ...
            # self.scroll.setMinimumWidth(1000)  # 设置最小宽度
            self.scroll2.setWidget(self.charts2)
            self.scroll2.adjustSize()

        except Exception as e:
            print(f"Error plotting data: {e}")

    def select_file_plt(self):
        # 弹出文件选择对话框
    ...
    def select_file_seaborn(self):
        # 弹出文件选择对话框
        file_path, _ = QFileDialog.getOpenFileName(self, "选择文件", "",
                                                   "All Files (*);;Python Files (*.py);;CSV Files (*.csv)")
        # 如果用户选择了文件，将路径显示在文本框中
        if file_path:
            self.seaborn_csv_data(file_path)
