...
def send_simple_message_start(type_, auto_close=False, auto_close_duration=1000):
...
def send_simple_message_end(type_, auto_close=False, auto_close_duration=1000):
    fold_after = auto_close_duration if auto_close is True else None
    SiGlobal.siui.windows["MAIN_WINDOW"].LayerRightMessageSidebar().send(
        "爬取完成了!\n"
        "保存为day7_weather_data.csv",
        msg_type=type_,
        fold_after=fold_after,
    )

def send_simple_message_success(type_, auto_close=False, auto_close_duration=1000):
    fold_after = auto_close_duration if auto_close is True else None
    SiGlobal.siui.windows["MAIN_WINDOW"].LayerRightMessageSidebar().send(
        "训练成功!\n"
        "模型保存到tensorflow_model文件夹中",
        msg_type=type_,
        fold_after=fold_after,
    )

def send_simple_message_fail(type_, auto_close=False, auto_close_duration=1000):
    fold_after = auto_close_duration if auto_close is True else None
    SiGlobal.siui.windows["MAIN_WINDOW"].LayerRightMessageSidebar().send(
        "训练失败!\n"
        "请检查tensorflow_train.py",
        msg_type=type_,
        fold_after=fold_after,
    )

def send_simple_message_train_start(type_, auto_close=False, auto_close_duration=1000):
    fold_after = auto_close_duration if auto_close is True else None
    SiGlobal.siui.windows["MAIN_WINDOW"].LayerRightMessageSidebar().send(
        "训练开始...\n"
        "加载中，数秒后开始",
...
def send_simple_message_predict_success(type_, auto_close=False, auto_close_duration=1000):
    fold_after = auto_close_duration if auto_close is True else None
    SiGlobal.siui.windows["MAIN_WINDOW"].LayerRightMessageSidebar().send(
        "预测成功！\n",
        msg_type=type_,
...
def send_simple_message_predict_fail(type_, auto_close=False, auto_close_duration=1000):
    fold_after = auto_close_duration if auto_close is True else None
    SiGlobal.siui.windows["MAIN_WINDOW"].LayerRightMessageSidebar().send(
        "预测失败！\n",
        "请检查模型文件夹",
