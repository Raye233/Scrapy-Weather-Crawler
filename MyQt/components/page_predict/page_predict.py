...
from .components.side_msg_box import send_simple_message_predict_success, send_simple_message_predict_fail
from ..option_card import OptionCardPlaneForWidgetDemos
from tensorflow_predict_api import WeatherPredictor
from pytorch_predict_api import predict_weather
...
class PredictWidgets(SiPage):
    ...
    def start_prediction(self):
        ...
        history_path = r'F:\Rayedata2\weather_crawl\beijing_year5_weather_data.csv'
        ...
        try:
            use_tensorflow = self.select_tensorflow.isChecked()
            ...
            if use_tensorflow:
                ...
                print(predict_date)
                print(history_path)
                results = predict_weather(model_dir, predict_date, history_path)

            self.show_prediction_result(results)
            send_simple_message_predict_success(self.message_type, self.message_auto_close, self.message_auto_close_duration)

        except Exception as e:
            QMessageBox.critical(self, "错误", f"天气预测失败：{str(e)}")
            send_simple_message_predict_fail(self.message_type, self.message_auto_close, self.message_auto_close_duration)
        finally:
