...
# # 使用示例
# prediction = predict_weather('2023-12-15')
# for k, v in prediction.items():
#     print(f'{k}: {v}')

model.save('weather_model.h5', save_format='h5')  # HDF5格式

# 保存预处理相关对象
joblib.dump(scaler_X, '../test_tensorflow_model/scaler_X.save')
joblib.dump(scaler_y, '../test_tensorflow_model/scaler_y.save')
joblib.dump(wind_dir_encoder, '../test_tensorflow_model/wind_dir_encoder.save')
joblib.dump(uv_encoder, '../test_tensorflow_model/uv_encoder.save')

# print("模型和预处理组件已保存".encode('utf-8').decode('gbk'))  # 确保中文兼容
# --------------------------------------------------
