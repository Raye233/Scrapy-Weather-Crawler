...
def train_model(model, train_loader, val_loader):
    ...
    for epoch in range(Config.epochs):
        ...
        if avg_val_loss < best_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), r'F:\Rayedata2\weather_crawl\pytorch_model\best_model.pth')
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load(r'F:\Rayedata2\weather_crawl\pytorch_model\best_model.pth'))
    return model
...
if __name__ == "__main__":
    ...
    def save_artifacts(scaler, targets, encoders, model):
        # 保存特征标准化器
        joblib.dump(scaler, "../pytorch_model/feature_scaler.save")
        # 保存目标变量标准化器
        joblib.dump(targets, "../pytorch_model/target_scaler.save")
        # 保存标签编码器
        encoder_data = {
            'wind_dir': encoders['wind_dir'].classes_.tolist(),
            'uv': encoders['uv'].classes_.tolist()
        }
        with open("../pytorch_model/label_encoders.json", 'w') as f:
            json.dump(encoder_data, f)

        # 保存模型
        torch.save(model.state_dict(), "../pytorch_model/best_weather_model.pth")
    # 在训练完成后调用保存函数
    save_artifacts(scaler, targets, encoders, trained_model)
