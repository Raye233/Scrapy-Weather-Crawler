...
import pandas as pd
from MyQt.global_vars import wind_dir_prob_dict, wind_force_prob_dict, uv_prob_dict
# 配置参数
class Config:
    seq_len = 5  # 时间窗口长度
    embed_dim = 32  # 嵌入维度
...
class PytorchWeatherPredictor:
    def __init__(self, model_dir):
        # 设置模型和预处理组件的路径
        model_path = os.path.join(model_dir, 'weather_model_lstm.pth')
        ...
        self.seasons = {1: 0, 2: 0, 3: 1, 4: 1, 5: 1, 6: 2, 7: 2, 8: 2, 9: 3, 10: 3, 11: 3, 12: 0}
        # 初始化并加载模型
        input_dim = 4  # 年、月、日、季节
        self.model = WeatherModel(input_dim=input_dim, cat_sizes={
            'wind_dir': len(self.wind_dir_encoder.classes_),
            'wind_force': 7,
            'uv': len(self.uv_encoder.classes_)
        })
        self.model.load_state_dict(torch.load(model_path))
        self.model.eval()

    def predict(self, date_str, history_data_path):
        # 读取数据
        df = pd.read_csv(history_data_path, encoding='utf-8')
        df['时间'] = pd.to_datetime(df['时间'])
        df['年'] = df['时间'].dt.year
        df['月'] = df['时间'].dt.month
        df['日'] = df['时间'].dt.day

        df['季节'] = df['月'].map(self.seasons)
        ...
        # 预测
        with torch.no_grad():
            predictions = self.model(torch.tensor(X_new_scaled).unsqueeze(0).float())
        ...
        numerical = self.scaler_y.inverse_transform(numerical).flatten()
        ...
        wind_dir_prob_dict.update(
            {wind_dir_labels[i]: f"{wind_dir_probs[0, i].item() * 100:.2f}%" for i in range(len(wind_dir_labels))})
        wind_force_prob_dict.update(
            {f"Force {wind_force_labels[i]}": f"{wind_force_probs[0, i].item() * 100:.2f}%" for i in
             range(len(wind_force_labels))})
        uv_prob_dict.update({uv_labels[i]: f"{uv_probs[0, i].item() * 100:.2f}%" for i in range(len(uv_labels))})
...
def predict_weather(model_dir, date_str, history_5days_path):
    predictor = PytorchWeatherPredictor(model_dir)
    result = predictor.predict(date_str, history_5days_path)
    return result
