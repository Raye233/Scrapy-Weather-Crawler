...
class WeatherPredictor:
    ...
    def predict(self, date_str):
        ...
        wind_dir_prob_dict.update({wind_dir_labels[i]: f"{wind_dir_probs[i] * 100:.2f}%" for i in
                                   range(len(wind_dir_labels))})
        wind_force_prob_dict.update({f"{wind_force_labels[i]}级": f"{wind_force_probs[i] * 100:.2f}%" for i
                                     in range(len(wind_force_labels))})
        uv_prob_dict.update({uv_labels[i]: f"{uv_probs[i] * 100:.2f}%" for i in range(len(uv_labels))})
